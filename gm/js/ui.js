(function () {
  ...
  function renderBookVisibilityPopup() {
    ...
    managerActions.className = 'book-visibility-manager-actions';

    const manageButton = document.createElement('button');
    manageButton.type = 'button';
    manageButton.className = 'primary';
    manageButton.textContent = 'Manage Rulebooks';
    manageButton.addEventListener('click', (event) => {
      event.preventDefault();
      event.stopPropagation();
      window.GM.popup?.hide?.();
      window.setTimeout(() => openRulebookManager(), 0);
    });

    managerActions.appendChild(manageButton);
    wrap.appendChild(managerActions);
    return wrap;
  }
  ...
  function serializeBooksConfig(books) {
    const ordered = Object.entries(books || {}).sort((a, b) => (Number(a[1]?.order) || 9999) - (Number(b[1]?.order) || 9999));
    const lines = [
      '// Generated by Game Master Rulebook Manager.',
      'window.BOOKS = {',
    ];
    ordered.forEach(([key, book], idx) => {
      const pages = Array.isArray(book.pages) ? book.pages : [];
      lines.push(`  ${JSON.stringify(key)}: {`);
      lines.push(`    order: ${idx + 1},`);
      lines.push(`    title: ${JSON.stringify(String(book.title || key))},`);
      lines.push(`    file: ${JSON.stringify(String(book.file || ''))},`);
      lines.push(`    pageOffset: ${Number(book.pageOffset) || 0},`);
      lines.push(`    defaultPage: ${Number(book.defaultPage) || 1},`);
      lines.push(`    defaultScale: ${Number(book.defaultScale) || 1.25},`);
      lines.push(`    defaultVisible: ${book.defaultVisible !== false ? 'true' : 'false'},`);
      lines.push(`    preload: ${book.preload === true ? 'true' : 'false'},`);
      lines.push('    pages: [');
      pages.forEach((page, pidx) => lines.push(`      { label: ${JSON.stringify(String(page.label || ''))}, page: ${Number(page.page) || 1} }${pidx < pages.length - 1 ? ',' : ''}`));
      lines.push('    ]');
      lines.push(`  }${idx < ordered.length - 1 ? ',' : ''}`);
    });
    lines.push('};', '');
    return lines.join('\n');
  }

  function applyBookConfig(books) {
    if (!books || typeof books !== 'object' || Array.isArray(books)) return false;
    window.BOOKS = books;
    buildTabs();
    const active = getCurrentTab();
    if (active) {
      buildPageButtons(active);
      setViewerTitle(active, getDisplayPage(active));
    }
    return true;
  }

  async function loadSharedBookConfig(raw) {
    try {
      const sandbox = {};
      const fn = new Function('window', `"use strict"; ${String(raw || '')}; return window.BOOKS;`);
      const books = fn(sandbox);
      return applyBookConfig(books);
    } catch (err) {
      console.warn('Could not load shared book config:', err);
      return false;
    }
  }

  function openRulebookManager() {
    const books = window.BOOKS || {};
    const wrap = document.createElement('div');
    wrap.className = 'rulebook-manager';

    const list = document.createElement('div');
    list.className = 'rulebook-manager-list';
    const status = document.createElement('div');
    status.className = 'rulebook-manager-status';

    const addRow = (key, book, original = true) => {
      const row = document.createElement('div');
      row.className = 'rulebook-manager-row';
      row._originalBook = original ? JSON.parse(JSON.stringify(book || {})) : {};

      const keyInput = document.createElement('input');
      keyInput.value = key;
      keyInput.placeholder = 'Book key';
      const titleInput = document.createElement('input');
      titleInput.value = book.title || key;
      titleInput.placeholder = 'Display name';
      const fileInput = document.createElement('input');
      fileInput.value = book.file || '';
      fileInput.placeholder = 'PDF path, e.g. pdfs/newbook.pdf';
      const offsetInput = document.createElement('input');
      offsetInput.type = 'number';
      offsetInput.value = Number(book.pageOffset) || 0;
      offsetInput.placeholder = 'Offset';
      const visibleLabel = document.createElement('label');
      const visibleInput = document.createElement('input');
      visibleInput.type = 'checkbox';
      visibleInput.checked = book.defaultVisible !== false;
      ...
      row.append(keyInput, titleInput, fileInput, offsetInput, visibleLabel, remove);
      list.appendChild(row);
    };

    Object.entries(books)
      .sort((a,b) => (Number(a[1]?.order)||9999) - (Number(b[1]?.order)||9999))
      .forEach(([key, book]) => addRow(key, book, true));

    const add = document.createElement('button');
    add.type = 'button';
    add.textContent = 'Add book';
    add.addEventListener('click', () => addRow(`book_${list.children.length + 1}`, {
      title: 'New Book', file: 'pdfs/newbook.pdf', pageOffset: 0,
      defaultPage: 1, defaultScale: 1.25, defaultVisible: false,
      preload: false, pages: [{ label: 'TOC', page: 1 }]
    }, false));
    ...
    const save = async (downloadOnly = false) => {
      ...
      Array.from(list.children).forEach((row, idx) => {
        ...
        const key = String(inputs[0]?.value || '').trim().replace(/[^A-Za-z0-9_-]+/g, '_');
        if (!key) return;
        result[key] = {
          ...(row._originalBook || {}),
          order: idx + 1,
          title: String(inputs[1]?.value || key).trim(),
          file: String(inputs[2]?.value || '').trim(),
          pageOffset: Number(inputs[3]?.value) || 0,
          defaultVisible: Boolean(inputs[4]?.checked),
        };
      });

      const configText = serializeBooksConfig(result);
      applyBookConfig(result);

      if (downloadOnly) {
        const blob = new Blob([configText], { type: 'application/javascript' });
        const url = URL.createObjectURL(blob);
        const anchor = document.createElement('a');
        anchor.href = url; anchor.download = 'config.js';
        document.body.appendChild(anchor); anchor.click(); anchor.remove();
        URL.revokeObjectURL(url);
        status.textContent = 'Downloaded config.js.';
        return;
      }

      const dataStatus = window.GM.data?.getStatus?.() || {};
      if (!dataStatus.connected || dataStatus.readOnly) {
        status.textContent = !dataStatus.connected
          ? 'Connect an editable data folder to save config.js.'
          : 'Server data is read-only. Download config.js and upload it to the server data store.';
        return;
      }
      try {
        await window.GM.data.writeFile('config.js', configText);
        status.textContent = 'Saved config.js to the data folder.';
      } catch (err) {
    ...
    saveButton.type = 'button'; saveButton.className = 'primary'; saveButton.textContent = 'Save config.js';
    saveButton.addEventListener('click', () => save(false));
    const downloadButton = document.createElement('button');
    downloadButton.type = 'button'; downloadButton.textContent = 'Download config.js';
    downloadButton.addEventListener('click', () => save(true));

    const help = document.createElement('p');
    help.className = 'book-visibility-intro';
    help.textContent = 'Add, rename, remove, or point rulebooks at PDFs in the pdfs folder. Existing page shortcuts and settings are preserved. Changes apply immediately.';

    const actions = document.createElement('div');
    actions.className = 'rulebook-manager-actions';
    actions.append(add, saveButton, downloadButton);

    wrap.append(help, list, actions, status);
    window.GM.popup?.show?.({
      title: 'Rulebook Manager',
      content: wrap,
      className: 'rulebook-manager-popup',
      width: 1180,
      resizable: true,
      closeOnScroll: false,
    });
